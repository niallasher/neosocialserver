...
def test_get_user_approvals(test_db, server_address):
    ...
    r = requests.get(
        f"{server_address}/api/v3/admin/userApprovals",
        json={
            "count": 10,
            "offset": 0,
...
def test_approve_user(test_db, server_address):
    ...
    r = requests.get(
        f"{server_address}/api/v3/username/available", json={"username": "user2"}
    )
...
def test_reject_user(test_db, server_address):
    ...
    # make sure the user still exists
    r = requests.get(
        f"{server_address}/api/v3/username/available", json={"username": "user22"}
    )
    assert r.status_code == 200
