from socialserver.util.test import server_address, create_user_session_with_request, test_db
import requests
...
def test_user_deauth_legacy(test_db, server_address):
    ...
    r = requests.post(f"{server_address}/api/v2/user/deauth",
                      json={
    ...
    for session_token in sessions:
        assert requests.get(f"{server_address}/api/v1/users",
                            json={
