...
def test_delete_user_admin_legacy(test_db, server_address):
    ...
    r = requests.delete(
        f"{server_address}/api/v1/admin/userdel",
        json={
    ...
    r = requests.get(
        ...
        json={"session_token": test_db.access_token, "username": "unwitting_victim"},
    )
...
def test_attempt_delete_other_admin_admin_legacy(test_db, server_address):
    ...
    create_user_with_request(username="unwitting_victim", password="password")
    set_user_attributes_db(
        test_db.db, "unwitting_victim", [AccountAttributes.ADMIN.value]
    )
    # destroy the aforementioned user with extreme prejudice
    r = requests.delete(
        f"{server_address}/api/v1/admin/userdel",
    ...
    assert (
            r.json()["err"]
            == LegacyErrorCodes.INSUFFICIENT_PERMISSIONS_TO_MODIFY_USER_DESTRUCTIVE.value
    )
