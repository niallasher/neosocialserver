...
def test_get_modqueue_legacy(test_db, server_address):
    ...
    r = requests.get(f"{server_address}/api/v1/modqueue",
                     json={
...
def test_get_not_mod_or_admin_modqueue_legacy(test_db, server_address):
    ...
    assert r.json()['err'] == LegacyErrorCodes.INSUFFICIENT_PERMISSIONS_TO_ACCESS_MODQUEUE.value
...
def test_remove_post_from_modqueue_legacy(test_db, server_address):
    ...
    r = requests.delete(f"{server_address}/api/v1/modqueue",
                        json={
                            "session_token": test_db.access_token,
                            "post_id": post_id
                        })
...
def test_try_get_moderated_post_not_mod_or_admin_legacy(test_db, server_address):
    ...
    assert r.status_code == 401
    assert r.json()['err'] == LegacyErrorCodes.INSUFFICIENT_PERMISSIONS_TO_VIEW_POST.value
