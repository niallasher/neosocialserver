...
def test_get_post_individual_invalid_id_legacy(test_db, server_address):
    ...
    r = requests.get(
        f"{server_address}/api/v1/posts",
        json={"session_token": test_db.access_token, "post_id": 377},
    )
...
def test_create_post_legacy(test_db, server_address):
    r = requests.post(
        f"{server_address}/api/v1/posts",
        json={"session_token": test_db.access_token, "post_text": "test"},
    )
    assert r.status_code == 201
...
def test_create_post_invalid_token_legacy(test_db, server_address):
    r = requests.post(
        f"{server_address}/api/v1/posts",
        json={"session_token": "invalid", "post_text": "test"},
    )
    assert r.status_code == 401
...
def test_create_post_too_long_legacy(test_db, server_address):
    r = requests.post(
        f"{server_address}/api/v1/posts",
        json={"session_token": test_db.access_token, "post_text": token_urlsafe(1024)},
    )
...
def test_create_post_missing_args_legacy(test_db, server_address):
    r = requests.post(
        f"{server_address}/api/v1/posts", json={"session_token": test_db.access_token}
    )
    assert r.status_code == 400
...
def test_delete_post_invalid_id_legacy(test_db, server_address):
    r = requests.delete(
        f"{server_address}/api/v1/posts",
        json={"session_token": test_db.access_token, "post_id": 1337},
    )
...
def test_try_delete_other_users_post_legacy(test_db, server_address):
    ...
    user_token = create_user_session_with_request(
...
def test_delete_post_invalid_token_legacy(test_db, server_address):
    ...
    r = requests.delete(
        f"{server_address}/api/v1/posts",
        json={"session_token": "abcxyz", "post_id": post_id},
    )
