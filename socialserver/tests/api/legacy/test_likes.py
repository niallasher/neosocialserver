...
def test_like_post_invalid_id_legacy(test_db, server_address):
    r = requests.post(
        f"{server_address}/api/v1/likes",
        json={"session_token": test_db.access_token, "post_id": 33},
    )
...
def test_get_like_count_no_likes_legacy(test_db, server_address):
    post_id = create_post_with_request(test_db.access_token)
    r = requests.get(
        f"{server_address}/api/v1/likes/byPost",
        json={
...
def test_get_like_info_legacy(test_db, server_address):
    ...
    r = requests.get(
        f"{server_address}/api/v1/likes",
        json={"session_token": test_db.access_token, "like_id": like_id},
    )
    assert r.status_code == 201
...
def test_get_like_info_invalid_id_legacy(test_db, server_address):
    r = requests.get(
        f"{server_address}/api/v1/likes",
        json={"session_token": test_db.access_token, "like_id": 1337},
    )
    assert r.status_code == 404
