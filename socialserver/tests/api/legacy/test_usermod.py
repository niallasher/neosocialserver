...
def test_update_display_name_legacy(test_db, server_address, monkeypatch):
    r = requests.post(
        f"{server_address}/api/v1/usermod",
        json={"session_token": test_db.access_token, "display_name": "new name"},
    )
...
def test_update_username_legacy(test_db, server_address, monkeypatch):
    r = requests.post(
        f"{server_address}/api/v1/usermod",
        json={"session_token": test_db.access_token, "username": "new_username"},
    )
    assert r.status_code == 201
    r2 = requests.get(
        f"{server_address}/api/v1/users",
...
def test_update_username_invalid_legacy(test_db, server_address):
    r = requests.post(
        f"{server_address}/api/v1/usermod",
        json={"session_token": test_db.access_token, "username": "#invalid!"},
    )
    assert r.status_code == 400
...
def test_update_avatar_legacy(test_db, server_address, image_data_binary):
    ...
    r = requests.post(
        f"{server_address}/api/v1/usermod",
        json={"session_token": test_db.access_token, "avatar_hash": image_identifier},
    )
    assert r.status_code == 201
    r2 = requests.get(
        f"{server_address}/api/v1/users",
