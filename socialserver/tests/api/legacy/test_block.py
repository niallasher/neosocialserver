...
def test_block_user_legacy(test_db, server_address):
    ...
    test2_session_token = create_user_session_with_request(
        username="test2", password="password"
    )
    post_id = create_post_with_request(
        auth_token=test2_session_token, text_content="bad opinion i disagree with"
    )
    # now block them
    r = requests.post(
        f"{server_address}/api/v1/block",
...
def test_get_block_list_no_blocked_users_legacy(test_db, server_address):
    r = requests.get(
        f"{server_address}/api/v1/users/blockList",
        json={"session_token": test_db.access_token},
    )
    assert len(r.json()) == 0
    assert r.status_code == 201
